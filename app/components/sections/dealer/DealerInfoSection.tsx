...
export function DealerInfoSection() {
  ...
  const pinDealerOnMap = (dealer: Dealer) => {
    ...
    // Use dealer name for geocoding (more accurate than coordinates from URL)
    if (geocoder) {
      console.log('Geocoding dealer name:', dealer.name);
      geocoder.geocode({ address: dealer.name }, (results: any, status: any) => {
        ...
        if (status === 'OK' && results && results[0]) {
          ...
          console.error('Geocoding failed for:', dealer.name, status);
          // Fallback to address if name fails
          geocoder.geocode({ address: dealer.address }, (results2: any, status2: any) => {
